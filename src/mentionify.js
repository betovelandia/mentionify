 ...
 * jQuery Mentionify
 * Version: 0.1
 * Description:  Plugin that allow the user to mention people ina textarea.
 * Author: Alberto Velandia. (http://github.com/betovelandia/)
 */
(function ($) {
  var keyCodes = {
    shift: 16,
  ...
  onKeyUp = function(e) {
    var text = $(this).val(),
      wrongKeyCodes = [keyCodes.enter, keyCodes.ctrl, keyCodes.alt, keyCodes.up,
                       keyCodes.down, keyCodes.shift],
      isMentioning = (text.charAt(text.length - 1) === '@');

    if (wrongKeyCodes.indexOf(e.keyCode) === -1 && isMentioning) {
      // Add logic if it is looking or not.
    }
  };


  $.fn.mentionify = function(url, options) {
      var opts = $.extend({}, $.fn.mentionify.defaults, options),
        isLooking = false,
        $textarea = $(this);
      ...
      $textarea.keyup(function(e) {
          text = this.value;
          if (e.keyCode != keycodes.enter && e.keyCode != keycodes.ctrl && e.keyCode != keycodes.alt && e.keyCode != keycodes.up && e.keyCode != keycodes.down && e.keycode != keycodes.shift) { // Unless alt or ctrl
              if (isLooking) {
